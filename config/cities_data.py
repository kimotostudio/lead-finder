#!/usr/bin/env python3
"""
Comprehensive city database for all 47 prefectures in Japan
Includes major cities (政令指定都市 + 中核市 + prefectural capitals)
"""

# Geographic regions with their prefectures
REGIONS = {
    '北海道': ['北海道'],
...
# Format: prefecture_name: [list of major cities]
# Includes: 政令指定都市 (ordinance-designated cities), 中核市 (core cities), and prefectural capitals
CITIES_BY_PREFECTURE = {
    ...
    '岩手県': ['盛岡市', '宮古市', '大船渡市', '花巻市', '北上市', '久慈市', '遠野市', '一関市'],
    ...
    # 関東地方
    '茨城県': ['水戸市', '日立市', '土浦市', '古河市', '石岡市', '結城市', '龍ケ崎市', '下妻市', '常総市', '常陸太田市', '高萩市', '北茨城市', '笠間市', '取手市', 'つくば市', 'ひたちなか市', '鹿嶋市', '潮来市', '守谷市', '那珂市', '筑西市', '坂東市', '稲敷市', 'かすみがうら市', '桜川市', '神栖市', '行方市', '鉾田市', 'つくばみらい市', '小美玉市'],
...
def get_all_prefectures():
    """Get list of all prefectures"""
    return sorted(CITIES_BY_PREFECTURE.keys())


def get_prefectures_by_region(region):
    """Get list of prefectures in a region"""
    return REGIONS.get(region, [])


def get_cities_by_prefecture(prefecture):
    """Get list of cities in a prefecture"""
    return CITIES_BY_PREFECTURE.get(prefecture, [])


def get_region_for_prefecture(prefecture):
    """Get the region for a given prefecture"""
...
def get_stats():
    ...
    total_cities = sum(len(cities) for cities in CITIES_BY_PREFECTURE.values())
    return {
        'total_regions': len(REGIONS),
        'total_prefectures': len(CITIES_BY_PREFECTURE),
        'total_cities': total_cities,
        'cities_per_region': {
            region: sum(len(CITIES_BY_PREFECTURE[pref]) for pref in prefs)
            for region, prefs in REGIONS.items()
        }
    }
...
if __name__ == '__main__':
    ...
    print(f"  Total Prefectures: {stats['total_prefectures']}")
    print(f"  Total Cities: {stats['total_cities']}")
    print(f"\nCities per Region:")
    for region, count in stats['cities_per_region'].items():
