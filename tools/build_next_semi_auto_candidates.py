...
def _evaluate_row(
    ...
    *,
    ...
    has_form = _truthy(_pick(row, ("has_form", "original__has_form", "生存")))
    has_contact_signal = bool(explicit_contact) or _truthy(_pick(row, ("has_contact_page", "original__has_contact_page")))
    ...
    if explicit_contact and direct_contact and good_contact_path and not toc_anchor:
        weak_contact = False
    if has_contact_signal is False and _pick(row, ("original__has_contact_page", "has_contact_page", "original__has_form", "has_form")):
        weak_contact = True
    ...
    if not include_ledger_domains and (lead_id in ledger_ids or _host_matches(domain, ledger_domains)):
        exclusion_reasons.append("duplicate_ledger")
    if (
        feedback_action in {"exclude", "block", "deprioritize"}
    ...
    row["semi_auto_quality_issue"] = ";".join(dict.fromkeys(issues))
    row["semi_auto_exclusion_reason"] = ";".join(dict.fromkeys(exclusion_reasons))
    row["semi_auto_contact_quality"] = (
        "direct" if direct_contact else "external" if external_contact else "non_web" if non_web_contact else "unknown"
    )
    ...
    row["recommended_action"] = feedback_action or ("prioritize_similar" if semi_auto_status == "prepared_full" else "")
    ...
    audit_row = {
        ...
        "contact_quality": row["semi_auto_contact_quality"],
    ...
    return CandidateEval(row=row, audit_row=audit_row, selected=not exclusion_reasons, quality_score=quality_score, exclusion_reasons=exclusion_reasons)
    ...
    *,
    ...
    for source_row, raw in enumerate(source_rows, 1):
        ...
        for reason in evaluated.exclusion_reasons:
            counts[f"excluded_{reason.split(':', 1)[0]}"] += 1

    selected = [item.row for item in evaluations if item.selected]
    selected.sort(key=lambda row: (int(row.get("semi_auto_quality_score") or 0), _to_float(row.get("score", ""))), reverse=True)
    if limit > 0:
